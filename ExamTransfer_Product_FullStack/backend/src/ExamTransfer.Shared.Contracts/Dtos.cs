...
public sealed record SubmissionFileDto(Guid Id, string Name, long SizeBytes, string Sha256, string MimeType, int TotalChunks, IReadOnlyList<int> ReceivedChunks, TransferStatus TransferStatus, string? DownloadUrl, bool ArchiveVerified = false);
public sealed record SubmissionSummaryDto(Guid Id, Guid SessionId, Guid ParticipantId, string StudentCode, string DisplayName, int AttemptNumber, SubmissionStatus Status, DateTimeOffset? ClientSubmittedAtUtc, DateTimeOffset? ServerReceivedAtUtc, DateTimeOffset DeadlineUtc, bool IsLate, string? ReceiptCode, bool IsOfficial, IReadOnlyList<SubmissionFileDto> Files, bool ComputedIsLate = false, bool? LateOverride = null);
