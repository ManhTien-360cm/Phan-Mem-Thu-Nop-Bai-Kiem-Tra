...
export async function handler(request: Request): Promise<Response> {
  ...
  if (!metadataResponse.ok) {
    return new Response(
      JSON.stringify({ error: "PUBLIC_EXAM_FILE_FORBIDDEN" }),
  ...
  if (rows.length !== 1 || !rows[0].object_path) {
    return new Response(
      JSON.stringify({ error: "PUBLIC_EXAM_FILE_NOT_FOUND" }),
  ...
  const signResponse = await fetch(
    `${supabaseUrl}/storage/v1/object/sign/exam-archives/${encodedPath}`,
    {
      method: "POST",
      headers: {
        ...jsonHeaders,
  ...
  if (!relativeUrl) {
    return new Response(JSON.stringify({ error: "SIGNED_URL_INVALID" }), {
      status: 502,
      headers: jsonHeaders,
    });
  }
