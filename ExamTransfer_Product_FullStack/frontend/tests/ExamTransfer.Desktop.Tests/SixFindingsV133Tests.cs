        ...
        string? terminalCode = null) : HttpMessageHandler
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (path.EndsWith("/rpc/get_examtransfer_cloud_capabilities", StringComparison.Ordinal))
                return Task.FromResult(Json("""{"schemaVersion":27,"criticalRpcs":["get_public_student_notification_events","send_public_teacher_message","get_student_results","set_public_submission_late_override"]}"""));
